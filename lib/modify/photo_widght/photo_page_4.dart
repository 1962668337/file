...
import 'package:flutter_file_manager/other_pages/photo_widght/photo_zoom_in.dart';
...
class _PhotoPage4State extends State<PhotoPage4> {
  ...
  Future<bool> onWillPop() async {
    if (currentDir.path != Common().rootPath + "/Pictures") // 如果当前路径不是根路径
    {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          // 返回根路径
          leading: currentDir?.path == Common().rootPath + "/Pictures"
              ? Container()
              : IconButton(
  ...
  Widget _buildFileItem(FileSystemEntity file) {
    return Container(
      child: InkWell(
        ...
        // 双击图片 跳转 三级页面
        onDoubleTap: () {
          Get.to(PhotoZoomIn(), arguments: file.parent);
        },
        // 长按图片 跳转 二级页面
        onLongPress: () {
        ...
        children: <Widget>[
          ...
          Container(
            decoration: BoxDecoration(
              ...
              image: DecorationImage(
                fit: BoxFit.cover,
                image: FileImage(File(file.path + "/1.jpg")),
              ),
            ),
          ),
